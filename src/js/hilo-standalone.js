     ...
     */
      browser: (function() {
        ...
        var jsVendor = (data.jsVendor = data.webkit
          ? 'webkit'
          : data.firefox ? 'moz' : data.opera ? 'o' : data.ie ? 'ms' : '')
     ...
     */
      _onDOMEvent: function(e) {
        ...
        if (target && ((target != obj && (!target.contains || !target.contains(obj))) || leave)) {
          var out =
            type === 'touchmove' ? 'touchout' : type === 'mousemove' || leave || !obj ? 'mouseout' : null
          if (out) {
     ...
     */
      start: function(useRAF) {
        ...
        var self = this,
          ...
          raf = window.requestAnimationFrame || window[Hilo.browser.jsVendor + 'RequestAnimationFrame']
...
;(function(window) {
   ...
   */
    var Elastic = createEase(
      ...
      function(k) {
        return (k *= 2) < 1
          ? -0.5 * (Elastic.a * pow(2, 10 * (k -= 1)) * sin((k - Elastic.s) * (2 * PI) / Elastic.p))
...
;(function(window) {
  ...
  var ImageLoader = Class.create({
    load: function(data) {
      ...
      image.onload = function() { //me.onLoad.bind(image);
        me.onLoad(image)
      }
